...
o.querySelector('.b615ms').onclick=async()=>{if(!files.length)return alert('Escolha uma foto ou vídeo');let s=db();s.moments||=[];for(const f of files){let media='';try{if(f.type.startsWith('image/')&&f.size<4500000)media=await dataURL(f);else if(f.type.startsWith('video/')&&f.size<2000000)media=await dataURL(f)}catch(e){}s.moments.push({id:'m-'+Date.now()+'-'+Math.random().toString(36).slice(2,8),tripId:(window.IPAData?.getActiveTrip?.()?.id||''),place:key,placeName:(b.dataset.placeName||'Momento da viagem'),name:f.name,type:f.type,media,caption:o.querySelector('.b615cap').value,featured:o.querySelector('.b615fav').checked,date:new Date(Date.now()+Math.random()*10).toISOString()})}try{save(s)}catch(e){return alert('A mídia ficou grande demais para salvar. Tente uma foto menor.')}o.remove();alert('Momento registrado e enviado para Memórias ✓');try{render()}catch(e){}} }
